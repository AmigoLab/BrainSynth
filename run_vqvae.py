...
def training(config: dict) -> None:
    ...
    if config["adversarial_component"]:
        ...
        if config["device"] == "ddp":
            d_network = DistributedDataParallel(
                d_network,
                device_ids=[config["local_rank"]],
                broadcast_buffers=False,
                bucket_cap_mb=25,
        ...
        if config["device"] == "ddp" and config["use_zero"]:
            d_optimizer = ZeroRedundancyOptimizer(
                d_network.parameters(),
                optimizer_class=torch.optim.Adam,
    ...
    if config["key_metric"] == "ms-ssim":
        config["ssim_win_size"] = get_ms_ssim_window(config=config, logger=logger)
        ms_ssim_kwargs = {"win_size": config["ssim_win_size"]}
        key_metric = {
    ...
    additional_metrics = {}
    if "mse" in config["additional_metrics"]:
        additional_metrics["Metric-MSE-Reconstruction"] = MSE(
            output_transform=lambda network_output: (
                network_output[CommonKeys.PRED]["reconstruction"][0],
                network_output[CommonKeys.IMAGE],
            )
        )
    if "mae" in config["additional_metrics"]:
        additional_metrics["Metric-MAE-Reconstruction"] = MAE(
            output_transform=lambda network_output: (
                network_output[CommonKeys.PRED]["reconstruction"][0],
                network_output[CommonKeys.IMAGE],
            )
        )
    if "ms-ssim" in config["additional_metrics"]:
        config["ssim_win_size"] = get_ms_ssim_window(config=config, logger=logger)
        ms_ssim_kwargs = {"win_size": config["ssim_win_size"]}
        additional_metrics["Metric-MS-SSIM_Reconstruction"] = MultiScaleSSIM(
            output_transform=lambda network_output: (
                network_output[CommonKeys.PRED]["reconstruction"][0],
                network_output[CommonKeys.IMAGE],
            ),
    ...
    if config["rank"] == 0:
        train_handlers += [
            VQVAELoggingHandler(
                ...
                log_image=True
                if config["data_spatial_dimension"] == 2
                and config["save_2d_images"] is not None
                else False,
            ),
    ...
    engine = SupervisedEvaluator(
        ...
        val_handlers=[
            TransformTraceLoggerHandler(
                ...
                trace_key="MRI_continuous_trace_dict",
            )
        ]
    ...
    if config["mode"] == VQVAEModes.EXTRACTING.value:
        if config["data_spatial_dimension"] == 2:
            batch_transform = lambda batch: {
                "filename_or_obj": batch["MRI_meta_dict"]["filename_or_obj"]
            }
        else:
            batch_transform = lambda batch: batch["MRI_meta_dict"]
        SegmentationSaver(
            output_dir=config["outputs_directory"],
            output_postfix="input",
            output_ext=".png" if config["data_spatial_dimension"] == 2 else ".nii.gz",
            resample=False,
            scale=255 if config["data_spatial_dimension"] == 2 else None,
            dtype=np.dtype(np.float32),
            batch_transform=batch_transform,
            output_transform=lambda output: output[CommonKeys.PRED]["input"],
        ).attach(engine)
        SegmentationSaver(
            output_dir=config["outputs_directory"],
...
def run(
    ...
    med3d_loss_depth: int = 50,
    med3d_checkpoint_path: str = None,
    ...
    discriminator_activation: str = "LEAKYRELU",
    discriminator_dropout: float = 0.0,
    discriminator_normalisation: str = "BATCH",
    ...
    # metrics
    key_metric: str = "ms-ssim",
    additional_metrics: Tuple[str, ...] = ("mae", "mse"),
    # Baur factor
    ...
    Args:
        ...
        cuda_enabled (bool): A bool that controls whether cuDNN is enabled. Defaults to True.
        ...
        med3d_loss_depth (int): The depth of the Med3D network which is used for perceptual loss. Defaults to 50.

        med3d_checkpoint_path (str): Path towards the Med3D checkpoint to be loaded. Defaults to None.
        ...
        discriminator_channels (int): The number of features channels the discriminator has. Defaults to 64.

        discriminator_layers (int): The number of layers the discriminator has. Defaults to 3.

        discriminator_activation (str): Which activation function is used in the discriminator. Defaults to 'LEAKYRELU'.

        discriminator_dropout (float): How much dropout is used in the discriminator. Defaults to 0.0.

        discriminator_normalisation (str): Which normalisation is used in the discriminator. Defaults to 'BATCH'.
        ...
        save_2d_images (Tuple[str,...]). Log 2D simages during training. Options are 'axial', 'sagittal', 'coronal'.If
            training on 2D data selecting any of these options will plot the full 2D image.
        ...
        data_num_channels (int): how many input/output channels for your data. Defaults to 1.

        key_metric (str): ms-ssim or mae. Defaults to mm-ssim.

        additional_metrics (Tuple[str,...]): Additional metrics to be computed during evaluation. Options are 'mae',
         'mse', 'ms-ssim'. Defaults to ('mae', 'mse')

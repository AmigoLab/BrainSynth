...
class TensorBoardHandler(object):
    ...
    Args:
        ...
        max_images (int): If working with 2D data this controls the maximum number of images to be saved in a collage in
    ...
    def log(self, engine: Engine) -> None:
        ...
        if TBSummaryTypes.IMAGES in summaries:
            for tag, img_tensor in summaries[TBSummaryTypes.IMAGES].items():
                self._writer.add_images(
                    tag=tag,
                    img_tensor=self._prepare_image(img_tensor)[:self.max_images,...],
                    global_step=step,
                )

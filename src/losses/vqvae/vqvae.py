...
# It is a torch.nn.Module to be able to move the network used for the perceptual loss to the desired compute devices
class JukeboxPerceptualSSIMLoss(torch.nn.Module):
    ...
    References:
        ...
        [3] Wang, Z., Bovik, A.C., Sheikh, H.R. and Simoncelli, E.P., 2004.
    ...
    def __init__(
        ...
        lpips_normalize: bool = True,
        fft_kwargs: Dict = None,
        is_ssim_ms: bool = True,
        ssim_win_size: int = 5,
        normalization_strategy: str = "scaling",
        input_range: str = "sigmoid",
    ):
        super(JukeboxPerceptualSSIMLoss, self).__init__()
        ...
        self.normalization_strategy = normalization_strategy.lower()
        assert (
            self.normalization_strategy in self.normalization_strategies,
            f"Got {self.normalization_strategy} but valid normalization strategies are {self.normalization_strategies}.",
        )

        self.input_range = input_range.lower()
        assert (
            self.input_range in self.input_ranges,
            f"Got {self.input_range} but valid input ranges are {self.input_ranges}.",
        )
    ...
    def forward(
        ...
        s_loss = 1 - self._get_structural_loss(
            original_images=y, perturbed_images=y_pred
        )

        self.summaries[TBSummaryTypes.SCALAR]["loss-Structural-Reconstruction"] = s_loss
    ...
    def _normalize_input(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.input_range == self.input_ranges[1]:
            x = x + 1
            x = x / 2

        # Min-Max Scaling Normalization
        if self.normalization_strategy == self.normalization_strategies[0]:
            x = x - torch.amin(x, dim=(2, 3, 4), keepdim=True)
            x = x / torch.amax(x, dim=(2, 3, 4), keepdim=True)
        # Clipping Normalization
        elif self.normalization_strategy == self.normalization_strategies[1]:
            x = torch.clamp(x, min=0, max=1)

        return x

    def _get_structural_loss(
        self, original_images: torch.Tensor, perturbed_images: torch.Tensor
    ) -> torch.Tensor:

        original_images = self._normalize_input(original_images)
        perturbed_images = self._normalize_input(perturbed_images)

        return self.ssim(original_images, perturbed_images)
...
class SSIMHartleyLoss(torch.nn.Module):
    # TODO: Write documentation
    input_ranges = ["sigmoid", "tanh"]
    normalization_strategies = [None, "scaling", "clipping"]
    ...
    def __init__(
        ...
        super(SSIMHartleyLoss, self).__init__()
...
class SSIMMed3DLoss(torch.nn.Module):
    # TODO: Write documentation
    input_ranges = ["sigmoid", "tanh"]
    normalization_strategies = [None, "scaling", "clipping"]
    ...
    def __init__(
        ...
        super(SSIMMed3DLoss, self).__init__()

        self.dimensions = dimensions

        self.include_pixel_loss = include_pixel_loss

        self.med3d = get_feature_extractor(
            {
                "network": FeatureExtractorNetworks.MEDNET3D.value,
                "depth": med3d_depth,
                "checkpoint_path": med3d_checkpoint_path,
            }
        )
    ...
    def forward(
        ...
        # ==== Perceptual Component ====
        p_loss = self._get_perceptual_loss(original_images=y, perturbed_images=y_pred)

        self.summaries[TBSummaryTypes.SCALAR][
            "loss-Med3_Perceptual-Reconstruction"
        ] = p_loss
    ...
    def _get_perceptual_loss(
        ...
        if self.input_range == self.input_ranges[0]:
            ori_img = original_images * 2 - 1
            per_img = perturbed_images * 2 - 1
        else:
            ori_img = original_images
            per_img = perturbed_images

        ori_img = self.med3d(ori_img)[0]
        per_img = self.med3d(per_img)[0]

        return F.l1_loss(per_img, ori_img)

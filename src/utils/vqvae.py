...
def get_transformations(
    ...
    data_num_channels: int,
    key: str = "MRI",
):
    assert not (
        standardize and normalize
    ), "standardize and normalize should not be turned on at the same time."

    augmentations = []
    augmentation_probability = augmentation_probability * int(
    ...
    if mode == VQVAEModes.DECODING.value:
        ...
        if data_num_channels == 1:
            transform += [AddChanneld(keys=[key])]
        ...
        if patch_size:
            ...
            augmentations += [
                RandFlipd(keys=[key], prob=augmentation_probability, spatial_axis=0),
                RandFlipd(keys=[key], prob=augmentation_probability, spatial_axis=1),
                RandFlipd(keys=[key], prob=augmentation_probability, spatial_axis=2),
                RandRotate90d(
        ...
        if standardize:
            transform += [
                ...
                ThresholdIntensityd(keys=[key], threshold=-1, above=True, cval=-1),
            ]
        if normalize:
            transform += [
                ThresholdIntensityd(keys=[key], threshold=1, above=False, cval=1.0),
...
def get_subjects(
    ...
    for path in paths:
        if os.path.isdir(path):
            files.append([os.path.join(path, os.fsdecode(f)) for f in os.listdir(path)])
    ...
    if mode == VQVAEModes.DECODING.value:
        ...
        for file in zip(files) if is_multi_level else files[0]:
            ...
            for idx, quantization in enumerate(file if is_multi_level else [file]):
                if quantization.endswith(".npy"):
                    valid_subject = (np.load(quantization) < num_embeddings[idx]).all()
        ...
        for file in files[0]:
            if file.endswith(".nii.gz") or file.endswith(".nii") or file.endswith(".npy"):
                if (
                ...
                raise ValueError(f"Path given is not a .nii.gz, .nii, or .npy file, but {file} ")

    if invalid_subjects > 0:
        logger.warning(
            f"{invalid_subjects} were discarded during data loading due to having tokens outside the configured range."
        )
...
def get_data_flow(
    ...
    The following fields are needed in config (in order of appearance):
        ...
        data_num_channels(int): The number of channels in the data. Defaults to 1.
    ...
    training_transform, _ = get_transformations(
        ...
        data_num_channels=config.get("data_num_channels", 1),
    ...
    if config["mode"] != VQVAEModes.DECODING.value:
        if config["data_spatial_dimension"] == 3:
            ...
            _, _, input_height, input_width= next(iter(training_loader))[
                "MRI"
            ].shape
            config["input_shape"] = (input_height, input_width)
